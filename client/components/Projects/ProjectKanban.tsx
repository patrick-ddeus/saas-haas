            ...
            <Card className="flex-1 flex flex-col">
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <CardTitle className="text-sm font-medium">
                    {stage.name || statusConfig[ stage.id ]?.name}
                  </CardTitle>
                  <Button
                    size="sm"
                    variant="ghost"
              ...
              <CardContent className="flex-1 space-y-3 p-3 pt-0">
                ...
                {getCurrentPageProjects(stage.projects, stage.id).map((project) => {
                  ...
                  const assignedTo = Array.isArray(project.assigned_to) ? project.assigned_to : [];
                  const tags = Array.isArray(project.tags) ? project.tags : [];

                  return (
                    <Card
                      key={project.id}
                      className={`cursor-move hover:shadow-md transition-shadow border-l-4 ${pinnedProjects.has(project.id) ? 'ring-2 ring-blue-200 bg-blue-50/50' : ''
                        }`}
                      style={{
                        borderLeftColor: statusConfig[ project.status ]?.color.includes('blue') ? '#3b82f6' :
